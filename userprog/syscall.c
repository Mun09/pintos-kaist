...
void check_address(void *uaddr)
{
  ...
  if (!uaddr || !is_user_vaddr (uaddr) || !find_page) {
	exit(-1);
  }
#else
	if (!is_user_vaddr(uaddr) || uaddr == NULL || !pml4_get_page(thread_current()->pml4, uaddr))
	{
		exit(-1);
	}
#endif
...
void check_with_spt(const char *file)
{
	void *addr = (void *)file;
#ifdef VM
