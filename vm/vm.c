...
vm_alloc_page_with_initializer (enum vm_type type, void *upage, bool writable,
	...
	if (spt_find_page (spt, upage) == NULL) {
		switch(type) {
			case VM_ANON:
				uninit_new(n_page, upage, init, VM_ANON, aux, anon_initializer);
				break;
			case VM_FILE:
				uninit_new(n_page, upage, init, VM_FILE, aux, file_backed_initializer);
				break;
			case VM_PAGE_CACHE:
				break;
		}
...
spt_find_page (struct supplemental_page_table *spt, void *va) {
	...
	page = page_lookup(va, &spt->spt_hash);
	return page;
}
...
spt_insert_page (struct supplemental_page_table *spt, struct page *page) {
	struct hash_elem *old = hash_insert(&spt->spt_hash, &page->h_elem);
	if(old != NULL) {
		return false;
	}
	return true;
